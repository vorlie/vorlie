...
const Clips: React.FC = () => {
  ...
  const sortedClips = useMemo(() => {
    ...
    return [...allClips].sort((a, b) =>
      sortOrder === "newest"
        ? parseDMY(b.date) - parseDMY(a.date)
        : parseDMY(a.date) - parseDMY(b.date),
    );
  }, [allClips, sortOrder]);

  const filteredClips = useMemo(
    () => filterClips(sortedClips, searchText),
      ...
      <div className="max-w-full mx-auto relative z-10 py-8">
        ...
        >
          <select
            ...
            className="bg-m3-surface-container/60 backdrop-blur-xl border border-m3-outline/20 rounded-none text-m3-on-surface text-xs font-black uppercase tracking-wider px-4 py-3 hover:border-m3-primary/30 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-m3-primary/30 cursor-pointer"
