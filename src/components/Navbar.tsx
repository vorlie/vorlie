...
const navLinks: NavItem[] = [
  ...
  },
  {
    ...
    icon: "auto_awesome",
    children: [
      { to: "https://discord.gg/yUueAFyAmN", label: "Miko's Shrine", icon: "temple_buddhist" },
      { to: "https://vorlie.pl/?link=invite_miko", label: "Invite Miko", icon: "person_add" },
...
export default function Navbar() {
  ...
  const updatePill = useCallback(() => {
    ...
    // 1. Priority: Mobile "More" button if menu is open
    ...
    // 3. Exact Link (Home/Blog)
    else if (linksRef.current[location.pathname]) {
      activeElement = linksRef.current[location.pathname];
    }
    // 4. Parent Category (Showcase, etc.)
    else {
      const activeGroup = navLinks.find(g => g.children?.some(c => c.to === location.pathname));
      if (activeGroup) {
        // Desktop check
        if (linksRef.current[activeGroup.label]) {
          activeElement = linksRef.current[activeGroup.label];
        }
        // Mobile check fallback to More
  ...
  const Icon = ({ name, className = "" }: { name: string; className?: string }) => (
    <span className={`material-symbols-rounded text-[20px] leading-none ${className}`} aria-hidden="true">{name}</span>
  );

  const renderChildLink = (link: NavItem) => {
    if (link.type === "divider") return <div key={Math.random()} className="my-2 border-t border-m3-outline/10" />;
    const isExternal = link.to?.startsWith("http");
    const childClass = "flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-[16px] transition-all duration-200";
    const content = (
      <>
        {link.icon && <Icon name={link.icon} />}
        <span className="flex-grow text-left">{link.label}</span>
        {isExternal && <Icon name="open_in_new" className="text-[16px] opacity-50" />}
      </>
    );

    if (isExternal) {
      return (
        <a key={link.to} href={link.to} target="_blank" rel="noopener noreferrer" className={`${childClass} text-m3-on-surface hover:bg-m3-on-surface/10`}>
          {content}
        </a>
      );
    }
    return (
      <NavLink key={link.to} to={link.to!} className={({ isActive }) => `${childClass} ${isActive ? "bg-m3-secondary text-m3-on-secondary" : "text-m3-on-surface hover:bg-m3-on-surface/10"}`}>
        {content}
      </NavLink>
    );
  };
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-50 px-4 flex justify-center pointer-events-none">
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-10 md:hidden pointer-events-auto" onClick={() => setIsMobileMenuOpen(false)}>
          <div className="absolute bottom-24 left-4 right-4 bg-m3-surface-container border border-m3-outline/20 rounded-[32px] p-2 shadow-2xl max-h-[70vh] overflow-y-auto animate-mobile-slide-up mx-auto max-w-lg" onClick={(e) => e.stopPropagation()}>
            {navLinks.slice(2).map((group) => (
              <div key={group.label} className="mb-2 last:mb-0">
                <div className="px-4 py-2 text-[11px] font-bold text-m3-primary uppercase tracking-widest opacity-70">{group.label}</div>
                <div className="flex flex-col gap-1">{group.children?.map((child) => renderChildLink(child))}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <nav ref={navRef} className="relative z-20 pointer-events-auto flex items-center gap-1 bg-m3-surface-container border border-m3-outline/20 rounded-[28px] shadow-lg px-2 py-2 transition-all duration-300">
        <div
            ...
            <div key={link.label} className="relative w-fit flex-shrink-0">
              <button
                ...
                className={`relative z-10 flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-full transition-all duration-300 ${
                  openDropdown === link.label || navLinks.find(g => g.label === link.label)?.children?.some(c => c.to === location.pathname)
              ...
              >
                {link.icon && <Icon name={link.icon} />}
                {link.label}
                <ChevronDownIcon className={`w-4 h-4 transition-transform duration-300 ${openDropdown === link.label ? "rotate-180" : ""}`} />
              </button>
              ...
              {openDropdown === link.label && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 min-w-[14rem] w-max max-w-[90vw] bg-m3-surface-container border border-m3-outline/20 rounded-[24px] shadow-2xl z-50 overflow-hidden animate-vertical-slide-in transform-gpu origin-bottom">
                  <div className="p-2 space-y-1">{link.children?.map((child) => renderChildLink(child))}</div>
                </div>
              )}
            </div>
          ))}
        ...
        <button
          ref={(el) => { linksRef.current["More"] = el; }}
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          className={`md:hidden flex items-center gap-2 px-4 py-2 rounded-full relative z-10 transition-all ${
            isMobileMenuOpen || navLinks.slice(2).some(g => g.children?.some(c => c.to === location.pathname))
              ? "text-m3-on-primary-container"
