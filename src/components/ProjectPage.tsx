  ...
  return (
    <div className="min-h-screen text-m3-on-surface animate-reveal">
      ...
      <div className="max-w-full mx-auto relative z-10 py-8">
          ...
          <div className="lg:w-2/3 flex flex-col gap-6">
            ...
            {data.warning && (
              <motion.div
                ...
                className="flex items-start gap-3 bg-amber-500/10 text-amber-400 px-5 py-4 rounded-2xl border border-amber-500/20"
              >
                <span className="material-symbols-rounded text-[20px] flex-shrink-0 mt-0.5">warning</span>
            ...
            >
              <a
                ...
                rel={data.primaryCTA.download ? undefined : "noopener noreferrer"}
                className="inline-flex items-center gap-2 bg-m3-primary text-m3-on-primary font-black uppercase tracking-tighter py-3 px-8 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 active:scale-95"
              >
            ...
            {data.features.filter(f => !f.subsections).length > 0 && (
              ...
              >
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {data.features.filter(f => !f.subsections).map((feature) => (
                    <div
                      key={feature.title}
                      className="p-5 bg-m3-on-surface/5 rounded-2xl border border-m3-outline/5 hover:bg-m3-on-surface/10 transition-colors duration-300"
                    >
                      <h3 className="text-sm font-black text-m3-primary mb-2">{feature.title}</h3>
                      {feature.desc && (
                        <p className="text-m3-on-surface-variant text-sm leading-relaxed">
                          {feature.links
                            ? feature.desc.split(feature.links[0].label).map((part, i, arr) =>
                                i < arr.length - 1 ? (
                                  <>
                                    {part}
                                    <a
                      ...
                      {feature.tags && (
                        <div className="flex flex-wrap gap-2 mt-2">
                          {feature.tags.map((tag) => (
                            <span
                              key={tag}
                              className="bg-m3-primary/10 text-m3-primary px-3 py-1 rounded-full text-[10px] font-black border border-m3-primary/20 tracking-wider uppercase"
                            >
                              {tag}
                            </span>
                          ))}
            ...
            {data.features.filter(f => f.subsections).map((feature, i) => (
              <motion.div
                key={feature.title}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {feature.subsections!.map((sub) => (
                    <div key={sub.title} className="p-5 bg-m3-on-surface/5 rounded-2xl border border-m3-outline/5">
                      <h3 className="text-sm font-black text-m3-primary mb-2">{sub.title}</h3>
                      <p className="text-m3-on-surface-variant text-sm leading-relaxed">{sub.desc}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
                ...
                {data.installSteps.map((platform) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 bg-m3-primary text-m3-on-primary rounded-xl flex items-center justify-center font-black text-lg shadow-md flex-shrink-0">
                        {platform.icon}
                      </div>
                      <h3 className="text-sm font-black text-m3-primary">{platform.platform}</h3>
                    </div>
                    ...
                    {platform.steps && (
                      <div className="space-y-2">
                        {platform.steps.map((step) => (
                          <div
                            key={step.label}
                            className="bg-m3-surface-container/50 rounded-xl p-3 border border-m3-outline/10 relative overflow-hidden group"
                          >
              ...
              <div className="flex flex-col gap-2">
                {data.techStack.map((tech) => (
                  <div key={tech} className="flex items-center gap-2.5">
                    <div className="w-1.5 h-1.5 rounded-full bg-m3-primary flex-shrink-0" />
                    <span className="font-bold text-xs text-m3-on-surface-variant">{tech}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                {data.gallery.map((img) => (
                  <div
                    key={img.path}
                    className="group relative bg-m3-on-surface/5 rounded-2xl overflow-hidden border border-m3-outline/5 hover:border-m3-primary/20 transition-all cursor-pointer"
                    ...
                    <div className="p-2">
                      ...
                      <div className="relative overflow-hidden rounded-xl">
                        ...
                        <div className="absolute inset-0 bg-m3-primary/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <div className="bg-m3-surface/90 p-2 rounded-full shadow-lg translate-y-2 group-hover:translate-y-0 transition-transform">
                            <span className="material-symbols-rounded text-m3-primary text-[20px]">zoom_in</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        >
          <div>
            ...
            <p className="text-m3-on-surface-variant text-sm">{data.acknowledgments}</p>
          </div>
      ...
      {modalImage && (
        ...
        >
          <div
            className="relative bg-m3-surface-container rounded-[40px] p-4 sm:p-6 shadow-2xl border border-m3-outline/20 max-w-6xl w-full flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              className="absolute top-6 right-6 z-10 p-3 bg-m3-primary text-m3-on-primary rounded-full shadow-xl hover:scale-110 active:scale-90 transition-all"
              onClick={() => setModalImage(null)}
            >
