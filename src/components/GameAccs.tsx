...
const GameAccs: React.FC = () => {
  return (
    <div className="">
      <h2 className="text-2xl font-bold text-m3-on-surface tracking-tight">
        Game Accounts
      </h2>
      <p className="text-m3-on-surface-variant mb-4">
        Here are my accounts for various games, if you're interested in playing
        with me!
      </p>
      <div className="space-y-6 w-full max-w-md">
        {GameAccsData.map((group) => (
          <div
            key={group.gameTitle}
            className="bg-m3-on-secondary/50 backdrop-blur-sm p-4 px-6 rounded-[24px] shadow-lg"
          >
            ...
            <ul className="space-y-1">
              {group.accounts.map((acc, index) => (
                ...
                >
                  ...
                  {acc.tag && (
                    <span className="text-m3-on-surface-variant ml-1">
                      #{acc.tag}
                    </span>
                  )}
                  {acc.region && (
                    <span className="text-m3-primary ml-2">({acc.region})</span>
                  )}
                  {acc.platform && (
                    <span className="text-m3-primary ml-2">
                      [{acc.platform}]
                    </span>
                  )}
                </li>
