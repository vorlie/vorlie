...
export const getAvatarUrl = (userId: string, avatarId: string | null): string => {
    if (!avatarId) {
        return `https://cdn.discordapp.com/embed/avatars/${Number(userId) % 5}.png`;
    }
    return `https://cdn.discordapp.com/avatars/${userId}/${avatarId}.${avatarId.startsWith('a_') ? 'gif' : 'png'}`;
};
